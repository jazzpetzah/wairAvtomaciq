...
public abstract class AbstractPagesCollection<B extends BasePage> {
    ...
    public B getCommonPage() throws Exception {
        if (pagesMapping.isEmpty()) {
            throw new IllegalStateException("No pages are created yet! Please set the first page first!");
        }
    ...
    public <T extends B> T getPage(Class<T> pageClass) throws Exception {
        if (!pagesMapping.containsKey(pageClass)) {
            ...
            pagesMapping.put(pageClass, getCommonPage().instantiatePage(pageClass));
        }
    ...
    public boolean hasPages() {
        return !this.pagesMapping.isEmpty();
    }
    ...
    public <T extends B> void setFirstPage(T page) {
        ...
        this.clearAllPages();
        log.debug(String.format("Setting the first page object to '%s'", page.getClass().getSimpleName()));
    ...
    public void printPages() {
        ...
        log.debug(String.format("%d page(s) are currently created:", pagesMapping.size()));
