...
class JavaXMBox implements ISupportsMessagesPolling {
    ...
    public void openFolder(final Folder folderToOpen, boolean shouldStartNotifier) throws MessagingException,
            ...
            do {
                try {
                    ...
                    if (((e instanceof MessagingException) || (e instanceof ProtocolException))
                        ...
                        log.error(String.format("Folder open operation failed: '%s' (retry %s of %s)...",
                                e.getMessage(), ntry + 1, MAX_OPEN_RETRIES));
        ...
        if (shouldStartNotifier && messagesCountNotifier == null) {
            ...
            messagesCountNotifier = new Thread() {
                ...
                public void run() {
                    ...
                    while (JavaXMBox.this.getFolder() != null && !this.isInterrupted()) {
                        try {
                            if (!JavaXMBox.this.getFolder().isOpen()) {
                                JavaXMBox.this.getFolder().open(Folder.READ_ONLY);
    ...
    protected void closeFolder(boolean shouldStopNotifier) throws MessagingException {
        this.closeFolder(this.getFolder(), shouldStopNotifier);
    }

    public void closeFolder(Folder folderToClose, boolean shouldStopNotifier) throws MessagingException {
        try {
    ...
    private static void initStore(String mboxUser, String mboxPassword) throws Exception {
        ...
        do {
            try {
                ...
                store.connect(MessagingUtils.getServerHost(), -1, mboxUser, mboxPassword);
                break;
            } catch (Exception e) {
                log.debug(String.format("Failed to connect to the mailbox (ntry %d of %d):",
    ...
    public JavaXMBox(String mboxUser, String mboxPassword) throws Exception {
        storeLock.acquire();
        try {
            if (store == null) {
                initStore(mboxUser, mboxPassword);
            }
        } finally {
            storeLock.release();
        }
    ...
    public void waitUntilMessagesCountReaches(String deliveredTo,
        ...
        SearchTerm term = new SearchTerm() {
            ...
            public boolean match(Message message) {
                try {
                    if (MessagingUtils.extractDeliveredToValue("", message).equals(deliveredTo)) {
                        return true;
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
        ...
        assert fetchedMsgsCount >= expectedMsgsCount : String
                .format("Count of messages delivered to %s is %s, but should be at least %s after %s second(s)",
                        deliveredTo, fetchedMsgsCount, expectedMsgsCount, timeoutSeconds);
    }
