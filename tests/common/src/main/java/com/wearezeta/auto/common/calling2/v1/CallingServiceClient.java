...
public class CallingServiceClient {
    ...
    private static final org.apache.log4j.Logger LOG = ZetaLogger.getLog(CallingServiceClient.class.getSimpleName());

    private static final String API_VERSION = "1";
    private static final boolean TRACE = false;
    private final InstanceResource INSTANCE_RESOURCE = new InstanceResource(getApiRoot(), API_VERSION, TRACE);

    private final CallResource CALL_RESOURCE = new CallResource(getApiRoot(), API_VERSION, TRACE);

    public Instance startInstance(ClientUser userAs, VersionedInstanceType instanceType, String name)
        throws CallingServiceInstanceException {
        InstanceRequest instanceRequest = new InstanceRequest(userAs.getEmail(), userAs.getPassword(), getBackendType(),
            instanceType, name);
        return INSTANCE_RESOURCE.createInstance(instanceRequest);
    }

    public Instance stopInstance(Instance instance) throws CallingServiceInstanceException {
        return INSTANCE_RESOURCE.destroyInstance(instance);
    }

    public InstanceStatus getInstanceStatus(Instance instance) throws CallingServiceInstanceException {
        return INSTANCE_RESOURCE.getInstance(instance).getStatus();
    }

    public Call acceptNextIncomingCall(Instance instance) throws CallingServiceCallException {
        CallRequest callRequest = new CallRequest();

        return CALL_RESOURCE.acceptNext(instance, callRequest);
    }

    public Call acceptNextIncomingVideoCall(Instance instance) throws CallingServiceCallException {
        CallRequest callRequest = new CallRequest();

        return CALL_RESOURCE.acceptNextVideo(instance, callRequest);
    }

    public Call callToUser(Instance instance, String convId) throws CallingServiceCallException {
        CallRequest callRequest = new CallRequest(convId);
        return CALL_RESOURCE.start(instance, callRequest);
    }

    public CallStatus getCallStatus(Instance instance, Call call) throws CallingServiceCallException {
        return CALL_RESOURCE.getCall(instance, call).getStatus();
    }

    public Call stopCall(Instance instance, Call call) throws CallingServiceCallException {
    ...
    private static BackendType getBackendType() {
        try {
            return BackendType.valueOf(CommonUtils.getBackendType(CallingServiceClient.class).toUpperCase());
        } catch (Exception ex) {
    ...
    private static String getApiRoot() {
        try {
            return CommonUtils.getDefaultCallingServiceUrlFromConfig(CallResource.class);
        } catch (Exception e) {
            e.printStackTrace();
