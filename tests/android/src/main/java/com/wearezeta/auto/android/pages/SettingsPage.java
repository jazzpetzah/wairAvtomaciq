...
public class SettingsPage extends AndroidPage {
	...
	public void enterSpotifyCredentials(String username, String password) throws Exception {
		DriverUtils.waitUntilLocatorIsDisplayed(getDriver(), By.id(idSpotifyWebView));
		DriverUtils.tapOnPercentOfElement(getDriver(), spotifyWebView, 50, percentageToSpotifyLoginButton);
		
		Thread.sleep(1000);
		AndroidCommonUtils.type(username);
		
		DriverUtils.tapOnPercentOfElement(getDriver(), spotifyWebView, 50, percentageToSpotifyPasswordField);
		
		Thread.sleep(1000);
		AndroidCommonUtils.type(password);
		
//		(new CommonAndroidSteps()).PressBackButton();
		
		DriverUtils.tapOnPercentOfElement(getDriver(), spotifyWebView, 50, percentageToSpotifyConfirmLogin);
	}
