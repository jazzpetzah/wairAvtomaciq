...
public class DeviceDetailPageSteps {
    ...
    private static final Function<String, String> NON_ENCRYPTED_DEVICE_HEADER_TEXT =
            username -> String.format("%s is using an old version of Wire. No devices are shown here.", username);
    ...
    private DeviceDetailPage getPage() throws Exception {
        return AndroidTestContextHolder.getInstance().getTestContext().getPagesCollection()
                .getPage(DeviceDetailPage.class);
    }
    ...
    public void IVerifyRememberedDeviceIdIsShown() throws Exception {
        ...
        Assert.assertEquals("Actual ID " + actualDeviceId + " is not equal to " + rememberedDeviceId,
                rememberedDeviceId, actualDeviceId);
    }
    ...
    public void ISeeNoEncryptedDeviceTextInHeaderForUser(String username) throws Exception {
        ...
        assertTrue(String.format("No encrypted device text in header of device detail page with value '%s' is not visible",
                expected), getPage().isHeaderTextVisible(expected));
    }
}
