...
public class GroupConnectedUserDetailsOverlayPageSteps {
     ...
     * @throws Exception
     * @step.^I( do not)? see (user name|unique user name|user info) of user (.*) on Group connected user details page$
     */
    @Then("^I( do not)? see (user name|unique user name|user info) of user (.*) on Group connected user details page$")
