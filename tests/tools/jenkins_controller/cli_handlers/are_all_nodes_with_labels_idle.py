...
class AreAllNodesWithLabelsIdle(CliHandlerBase):
    def _build_options(self, parser):
        super(AreAllNodesWithLabelsIdle, self)._build_options(parser)
    ...
    def __call__(self):
        ...
        if online_nodes_count == 0:
            raise NoMatchesError('There are no online nodes with label(s) {0}'.\
                                 format(args.labels))
        return True
