...
public class PhoneNumberVerificationPageSteps {
    ...
    public void IEnterPhoneVerificationCodeForUser(String name) throws Throwable {
    ...
    public void IEnterPhoneVerificationCodeForEmaillessUser(String name) throws Throwable {
        ClientUser user = context.getUserManager().findUserByNameOrNameAlias(name);
        String code = BackendAPIWrappers.getLoginCodeByPhoneNumber(user.getPhoneNumber());
        context.getWebappPagesCollection().getPage(PhoneNumberVerificationPage.class).enterCodeForEmaillessUser(code);
    }

    @When("^I enter wrong phone verification code for user (.*)$")
    public void i_enter_wrong_phone_verification_code_for_user_user_Name(String name) throws Throwable {
        ClientUser user = context.getUserManager().findUserByNameOrNameAlias(name);
        String code = BackendAPIWrappers.getLoginCodeByPhoneNumber(user.getPhoneNumber());
        String wrongcode = "";
        if (code.charAt(0) != '1') {
        ...
        context.getWebappPagesCollection().getPage(PhoneNumberVerificationPage.class).enterCode(wrongcode);
    }

    @Then("^I see invalid phone code error message saying (.*)")
    public void TheSignInErrorMessageReads(String message) throws Exception {
        assertThat("invalid phone code error", context.getWebappPagesCollection().getPage(PhoneNumberVerificationPage.class)
