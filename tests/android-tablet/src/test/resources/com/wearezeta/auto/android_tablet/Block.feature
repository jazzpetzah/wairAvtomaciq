Feature: Block
  ...
  Scenario Outline: (AN-2798) I want to unblock someone from pop-over opened from search (landscape)
    ...
    When I tap the found item <Contact> on Search page
  ...
  Scenario Outline: (AN-4282 for android 4.4, AN-4538) I block user from 1:1 pop-over and can see blocked user in search results with blocked badge (landscape)
    Given There are 2 users where <Name> is me
    Given <Contact> is connected to me
    Given User Myself blocks user <Contact>
    Given I rotate UI to landscape
