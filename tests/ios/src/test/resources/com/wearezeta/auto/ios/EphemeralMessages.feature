Feature: Ephemeral Messages
  ...
  Scenario Outline: Group - Verify the message is deleted on the sender side when it's read on the receiver side
    Given There are 3 users where <Name> is me
    Given Myself is connected to <Contact1>,<Contact2>
    Given Myself has group chat <GroupChatName> with <Contact1>,<Contact2>
    Given I sign in using my email or phone number
    ...
    And I verify the remembered message has been changed in the local database
# Waiting for Dean's fix for new SE actor
    ...
    Examples:
      | Name      | Contact1   | Contact2  | GroupChatName | Timeout |
      | user1Name | user2Name  | user3Name | Epheme grp    | 30      |

  @C318634 @regression @fastLogin
  Scenario Outline: Group - Verify the message is deleted on the receiver side when timer is over
    Given There are 3 users where <Name> is me
    Given Myself is connected to <Contact1>,<Contact2>
    Given Myself has group chat <GroupChatName> with <Contact1>,<Contact2>
    Given I sign in using my email or phone number
  ...
  Scenario Outline: Group - Verify receiving ephemeral assets (picture, video, audio, link preview, location)
    Given There are 3 users where <Name> is me
    Given Myself is connected to <Contact1>,<Contact2>
    Given Myself has group chat <GroupChatName> with <Contact1>,<Contact2>
    ...
    Given I see conversations list
    Given I tap on group chat with name <GroupChatName>
    Given User <Contact1> switches user <Contact1> to ephemeral mode with <EphemeralTimeout> seconds timeout
    # Picture
    ...
    Then I do not see link preview container in the conversation view
    # Location
    When User <Contact1> shares the default location to group conversation <GroupChatName> via device <DeviceName>
    And I wait for <SyncTimeout> seconds
    And I see location map container in the conversation view
    And I wait for <EphemeralTimeout> seconds
    Then I do not see location map container in the conversation view
    ...
    Examples:
      ...
      | user1Name | user2Name | user3Name | 3           | 5                | ContactDevice | Epheme grp    | testing.jpg | testing.mp4 | video/mp4 | test.m4a      | audio/mp4 | www.wire.com |