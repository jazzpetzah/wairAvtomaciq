...
public class ContactListPage extends WebPage {
     ...
     * @throws Exception
     */
    protected String fixDefaultGroupConvoName(String conversationName, boolean includeArchived, boolean throwOnError)
        throws Exception {
        if (conversationName.contains(DEFAULT_GROUP_CONVO_NAMES_SEPARATOR)) {
            final Set<String> initialNamesSet = new HashSet<String>(Arrays.asList(conversationName.split(","))).stream()
                .map(x -> x.trim()).collect(Collectors.toSet());
            List<WebElement> convoNamesToCheck = new ArrayList<WebElement>();
            if (DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(),
                By.xpath(WebAppLocators.ContactListPage.xpathContactListEntries), 3)) {
            ...
            for (WebElement convoItem : convoNamesToCheck) {
                ...
                final Set<String> convoItemNamesSet = new HashSet<String>(Arrays.asList(convoName.split(","))).stream()
                    .map(x -> x.trim()).collect(Collectors.toSet());
            ...
            if (throwOnError) {
                throw new RuntimeException(String.format("Group conversation '%s' does not exists in the list",
                    conversationName));
            } else {
                return null;
            }
    ...
    private String fixDefaultGroupConvoName(String conversationName, boolean includeArchived) throws Exception {
    ...
    public void waitForSelfProfileAvatar() throws Exception {
        assert DriverUtils.waitUntilElementClickable(this.getDriver(), selfProfileAvatar);
    }
    ...
    public boolean isConvoListEntryWithNameExist(String name) throws Exception {
        ...
        return DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), By.cssSelector(locator), 5);
    }

    public boolean isArchiveListEntryWithNameExist(String name) throws Exception {
        log.debug("Looking for contact with name '" + name + "'");
        name = fixDefaultGroupConvoName(name, true, false);
        if (name == null) {
            return false;
        }
        final String locator = WebAppLocators.ContactListPage.cssArchiveListEntryByName.apply(name);
        return DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), By.cssSelector(locator), 5);
    }
    ...
    public boolean isConvoListEntryNotVisible(String name) throws Exception {
        ...
        final String locator = WebAppLocators.ContactListPage.cssContactListEntryByName.apply(name);
        return DriverUtils.waitUntilLocatorDissapears(this.getDriver(), By.cssSelector(locator), 5);
    }

    public WebElement getListElementByName(String name, boolean includeArchived) throws Exception {
    ...
    public boolean isMissedCallVisibleForContact(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
        final String locator = WebAppLocators.ContactListPage.xpathMissedCallNotificationByContactName.apply(conversationName);
        return DriverUtils.waitUntilLocatorIsDisplayed(getDriver(), By.xpath(locator));
    }

    public boolean isMissedCallInvisibleForContact(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
        final String locator = WebAppLocators.ContactListPage.xpathMissedCallNotificationByContactName.apply(conversationName);
        return DriverUtils.waitUntilLocatorDissapears(getDriver(), By.xpath(locator));
    }

    public boolean isJoinedGroupCallNotificationVisibleForConversation(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
    ...
    public boolean isJoinedGroupCallNotificationInvisibleForConversation(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
        final String locator = WebAppLocators.ContactListPage.xpathJoinedGroupCallNotificationByConversationName
            .apply(conversationName);
        return DriverUtils.waitUntilLocatorDissapears(getDriver(), By.xpath(locator));
    ...
    public boolean isUnjoinedGroupCallNotificationVisibleForConversation(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
    ...
    public boolean isUnjoinedGroupCallNotificationInvisibleForConversation(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
        final String locator = WebAppLocators.ContactListPage.xpathUnjoinedGroupCallNotificationByConversationName
            .apply(conversationName);
        return DriverUtils.waitUntilLocatorDissapears(getDriver(), By.xpath(locator));
    ...
    public void openArchive() throws Exception {
        this.getWait().until(ExpectedConditions.elementToBeClickable(openArchivedConvosButton));
    ...
    private void waitForOptionButtonsToBeClickable() throws Exception {
        ...
        assert (DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(),
            By.cssSelector(WebAppLocators.ContactListPage.cssMuteButton), 3) && DriverUtils.waitUntilElementClickable(
            this.getDriver(), muteButton, 3))
            || (DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(),
                By.cssSelector(WebAppLocators.ContactListPage.cssUnmuteButton), 3) && DriverUtils.waitUntilElementClickable(
    ...
    public boolean isConversationMuted(String conversationName) throws Exception {
        ...
        return DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(),
            By.xpath(WebAppLocators.ContactListPage.xpathMuteIconByContactName.apply(conversationName)), 5);
    }

    public boolean isConversationNotMuted(String conversationName) throws Exception {
        // moving focus from contact - to now show ... button
        // do nothing (safari workaround)
        if (WebAppExecutionContext.getBrowser().isSupportingNativeMouseActions()) {
            DriverUtils.moveMouserOver(this.getDriver(), selfProfileAvatar);
        ...
        return DriverUtils.waitUntilLocatorDissapears(this.getDriver(),
            By.xpath(WebAppLocators.ContactListPage.xpathMuteIconByContactName.apply(conversationName)));
    }

    public void clickOptionsButtonForContact(String conversationName) throws Exception {
        if (!WebAppExecutionContext.getBrowser().isSupportingNativeMouseActions()) {
            DriverUtils.addClassToParent(this.getDriver(), getListElementByName(conversationName, false), "hover");
        } else {
            DriverUtils.moveMouserOver(this.getDriver(), getListElementByName(conversationName, false));
        }
        conversationName = fixDefaultGroupConvoName(conversationName, false);
        ...
        optionsButton.click();
        if (!WebAppExecutionContext.getBrowser().isSupportingNativeMouseActions()) {
            DriverUtils.removeClassFromParent(this.getDriver(), getListElementByName(conversationName, false), "hover");
        }
    }
    ...
    private void waitUtilEntryIsSelected(final WebElement entry) throws Exception {
        Wait<WebDriver> wait = new FluentWait<WebDriver>(this.getDriver()).withTimeout(SELECTION_TIMEOUT, TimeUnit.SECONDS)
    ...
    public void clickWithJS(String cssSelector) throws Exception {
        this.getDriver().executeScript(String.format("$(document).find(\"%s\").click();", cssSelector));
    }
    ...
    public void openConversation(String conversationName) throws Exception {
        ...
        final By entryLocator = By
            .cssSelector(WebAppLocators.ContactListPage.cssContactListEntryByName.apply(conversationName));
        assert DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), entryLocator, OPEN_CONVO_LIST_ENTRY_TIMEOUT) : "Conversation item '"
            + conversationName
            + "' has not been found in the conversations list within "
            + OPEN_CONVO_LIST_ENTRY_TIMEOUT
            + " second(s) timeout";
        selectEntryWithRetry(entryLocator, WebAppLocators.ContactListPage.cssContactListEntryByName.apply(conversationName));
    }

    public void openConnectionRequestsList() throws Exception {
        final By entryLocator = By.cssSelector(WebAppLocators.ContactListPage.cssIncomingPendingConvoItem);
        assert DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), entryLocator, OPEN_CONVO_LIST_ENTRY_TIMEOUT) : "Incoming connection requests entry has not been found within "
            + OPEN_CONVO_LIST_ENTRY_TIMEOUT + " second(s) timeout";
        selectEntryWithRetry(entryLocator, WebAppLocators.ContactListPage.cssIncomingPendingConvoItem);
    }
    ...
    public void unarchiveConversation(String conversationName) throws Exception {
        ...
        final By archivedEntryLocator = By.xpath(WebAppLocators.ContactListPage.xpathArchivedContactListEntryByName
            .apply(conversationName));
        assert DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), archivedEntryLocator, 3);
        final WebElement archivedEntry = this.getDriver().findElement(archivedEntryLocator);
        archivedEntry.click();

        final By unarchivedEntryLocator = By.cssSelector(WebAppLocators.ContactListPage.cssContactListEntryByName
            .apply(conversationName));
        assert DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), unarchivedEntryLocator, 3);
        final WebElement unarchivedEntry = this.getDriver().findElement(unarchivedEntryLocator);
        waitUtilEntryIsSelected(unarchivedEntry);
    }

    public String getIncomingPendingItemText() throws Exception {
        final By entryLocator = By.cssSelector(WebAppLocators.ContactListPage.cssIncomingPendingConvoItem);
        assert DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(), entryLocator) : "There are no visible incoming pending connections in the conversations list";
        return getDriver().findElement(entryLocator).getText();
    }

    public boolean isSelfNameEntrySelected() throws Exception {
        final By locator = By.cssSelector(WebAppLocators.ContactListPage.cssSelfProfileAvatar);
        waitForSelfProfileAvatar();
        final WebElement entry = getDriver().findElement(locator);
        try {
            waitUtilEntryIsSelected(entry);
    ...
    public boolean isConversationSelected(String convoName) throws Exception {
        ...
        final By locator = By.cssSelector(WebAppLocators.ContactListPage.cssContactListEntryByName.apply(convoName));
    ...
    public int getItemIndex(String convoName) throws Exception {
        convoName = fixDefaultGroupConvoName(convoName, false);
        final int entriesCount = this.getDriver()
            .findElements(By.xpath(WebAppLocators.ContactListPage.xpathContactListEntries)).size();
        for (int entryIdx = 1; entryIdx <= entriesCount; entryIdx++) {
            final WebElement entryElement = this.getDriver().findElement(
                By.xpath(WebAppLocators.ContactListPage.xpathContactListEntryByIndex.apply(entryIdx)));
            if (entryElement.getAttribute("data-uie-value").equals(convoName)) {
                return entryIdx;
            }
        }
        throw new AssertionError(String.format("There is no '%s' conversation in the list", convoName));
    }

    public void waitUntilArchiveButtonIsNotVisible(int archiveBtnVisilityTimeout) throws Exception {
        assert DriverUtils.waitUntilLocatorDissapears(this.getDriver(),
            By.xpath(WebAppLocators.ContactListPage.xpathOpenArchivedConvosButton), archiveBtnVisilityTimeout) : "Open Archive button is still visible after "
            + archiveBtnVisilityTimeout + " second(s)";
    }

    public void waitUntilArchiveButtonIsVisible(int archiveBtnVisilityTimeout) throws Exception {
        assert DriverUtils.waitUntilLocatorIsDisplayed(this.getDriver(),
            By.xpath(WebAppLocators.ContactListPage.xpathOpenArchivedConvosButton), archiveBtnVisilityTimeout) : "Open Archive button is not visible after "
            + archiveBtnVisilityTimeout + " second(s)";
    }
    ...
    public boolean isPingIconVisibleForConversation(String conversationName) throws Exception {
        final By locator = By.xpath(WebAppLocators.ContactListPage.xpathPingIconByContactName.apply(conversationName));
    ...
    public void pressShortCutToMuteOrUnmute(String conversationName) throws Exception {
        conversationName = fixDefaultGroupConvoName(conversationName, false);
    ...
    // only for Wrapper
    public void openContextMenuForContact(String name) throws Exception {
        if (name == null) {
