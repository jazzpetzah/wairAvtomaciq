...
public class ContactListPageSteps {
    ...
    public void ISeeMyNameOnTopOfContactList() throws Exception {
        Assert.assertTrue("No contact list loaded.", webappPagesCollection.getPage(ContactListPage.class)
            .waitForContactListVisible());
        webappPagesCollection.getPage(ContactListPage.class).waitForSelfProfileAvatar();
    }
    ...
    public void GivenISeeContactListWithName(String name) throws Exception {
        ...
        Assert.assertTrue("No contact list loaded.", webappPagesCollection.getPage(ContactListPage.class)
            .waitForContactListVisible());
        for (int i = 0; i < 5; i++) {
            if (webappPagesCollection.getPage(ContactListPage.class).isConvoListEntryWithNameExist(name)) {
                return;
            }
            Thread.sleep(1000);
        }
    ...
    public void ISeeConversationIsSelected(String convoName) throws Exception {
        ...
        Assert.assertTrue(String.format("Conversation '%s' should be selected", convoName),
            webappPagesCollection.getPage(ContactListPage.class).isConversationSelected(convoName));
    }
    ...
    public void GivenIUnarchiveConversation(String name) throws Exception {
        name = usrMgr.replaceAliasesOccurences(name, FindBy.NAME_ALIAS);
        webappPagesCollection.getPage(ContactListPage.class).unarchiveConversation(name);
    }
    ...
    public void IDoNotSeeContactListWithName(String name) throws Exception {
        name = usrMgr.replaceAliasesOccurences(name, FindBy.NAME_ALIAS);
        Assert.assertTrue(webappPagesCollection.getPage(ContactListPage.class).isConvoListEntryNotVisible(name));
    }
    ...
    public void IDoNotSeeXIncomingConnection(String doNot, int amount) throws Exception {
        if (doNot == null) {
            if (amount == 1) {
                assertThat(webappPagesCollection.getPage(ContactListPage.class).getIncomingPendingItemText(),
                    equalTo(WebAppLocators.Common.CONTACT_LIST_ONE_PERSON_WAITING));
            } else {
                assertThat(webappPagesCollection.getPage(ContactListPage.class).getIncomingPendingItemText(), equalTo(amount
                    + WebAppLocators.Common.CONTACT_LIST_X_PEOPLE_WAITING));
            }
        } else {
            String itemText = "";
            try {
                itemText = webappPagesCollection.getPage(ContactListPage.class).getIncomingPendingItemText();
    ...
    public void IOpenIncomingConnectionRequestsList() throws Exception {
        webappPagesCollection.getPage(ContactListPage.class).openConnectionRequestsList();
    }
    ...
    public void ISeeConversationIsMuted(String contact) throws Exception {
        ...
        Assert.assertTrue(webappPagesCollection.getPage(ContactListPage.class).isConversationMuted(contact));
    }
    ...
    public void ISeeConversationIsNotMuted(String contact) throws Exception {
        ...
        Assert.assertFalse(webappPagesCollection.getPage(ContactListPage.class).isConversationMuted(contact));
    }
    ...
    public void IVerifyContactIndex(String convoNameAlias, int expectedIndex) throws Exception {
        convoNameAlias = usrMgr.replaceAliasesOccurences(convoNameAlias, FindBy.NAME_ALIAS);
        final int actualIndex = webappPagesCollection.getPage(ContactListPage.class).getItemIndex(convoNameAlias);
    ...
    public void IVerifyArchiveButtonVisibility(String shouldNotBeVisible) throws Exception {
        if (shouldNotBeVisible == null) {
            webappPagesCollection.getPage(ContactListPage.class).waitUntilArchiveButtonIsVisible(ARCHIVE_BTN_VISILITY_TIMEOUT);
        } else {
            webappPagesCollection.getPage(ContactListPage.class).waitUntilArchiveButtonIsNotVisible(
    ...
    public void isCallMissedVisibleForContact(String shouldNotBeVisible, String conversationName) throws Exception {
        ...
        if (shouldNotBeVisible == null) {
            assertTrue(String.format("The call notification in conversation '%s' should be visible", conversationName),
                webappPagesCollection.getPage(ContactListPage.class).isMissedCallVisibleForContact(conversationName));
        } else {
            assertTrue(String.format("The call notification in conversation '%s' should NOT be visible", conversationName),
                webappPagesCollection.getPage(ContactListPage.class).isMissedCallInvisibleForContact(conversationName));
        }
    }
    ...
    public void isJoinedGroupCallNotificationVisibleForConversation(String shouldNotBeVisible, String conversationName)
        throws Exception {
        conversationName = usrMgr.replaceAliasesOccurences(conversationName, FindBy.NAME_ALIAS);
        if (shouldNotBeVisible == null) {
            assertTrue(
                String.format("The joined group call notification in conversation '%s' should be visible", conversationName),
                webappPagesCollection.getPage(ContactListPage.class).isJoinedGroupCallNotificationVisibleForConversation(
                    conversationName));
        } else {
            assertTrue(String.format("The joined group call notification in conversation '%s' should NOT be visible",
                conversationName), webappPagesCollection.getPage(ContactListPage.class)
    ...
    public void isUnjoinedGroupCallNotificationVisibleForConversation(String shouldNotBeVisible, String conversationName)
        throws Exception {
        conversationName = usrMgr.replaceAliasesOccurences(conversationName, FindBy.NAME_ALIAS);
        if (shouldNotBeVisible == null) {
            assertTrue(
                String.format("The unjoined group call notification in conversation '%s' should be visible", conversationName),
                webappPagesCollection.getPage(ContactListPage.class).isUnjoinedGroupCallNotificationVisibleForConversation(
                    conversationName));
        } else {
    ...
    /*
     * Verify if ping icon in contact list in conversation with user is colored to expected accent color
     * 
     * @step. "^I verify ping icon in conversation with (\\w+) has (\\w+) color$"
     * 
    ...
    public void IVerifyPingIconColor(String conversationName, String colorName) throws Exception {
        ...
        final AccentColor pingIconColor = webappPagesCollection.getPage(ContactListPage.class).getCurrentPingIconAccentColor(
            conversationName);
        Assert.assertEquals(expectedColor, pingIconColor);
    }

    /*
     * Verify if unread dot in contact list in conversation with user is colored to expected accent color
     * 
     * @step. "^I verify unread dot in conversation with (\\w+) has (\\w+) color$"
     * 
     * @param colorName one of these colors: StrongBlue, StrongLimeGreen, BrightYellow, VividRed, BrightOrange, SoftPink, Violet
     * 
     * @throws Exception
    ...
    public void IVerifyUnreadDotColor(String conversationName, String colorName) throws Exception {
        conversationName = usrMgr.replaceAliasesOccurences(conversationName, FindBy.NAME_ALIAS);
        final AccentColor expectedColor = AccentColor.getByName(colorName);
        final AccentColor unreadDotColor = webappPagesCollection.getPage(ContactListPage.class).getCurrentUnreadDotAccentColor(
            conversationName);
    ...
    public void ISeePingIcon(String conversationName) throws Exception {
        conversationName = usrMgr.replaceAliasesOccurences(conversationName, FindBy.NAME_ALIAS);
        Assert.assertTrue("No ping visible.", webappPagesCollection.getPage(ContactListPage.class)
    ...
    public void ISeePreviouslyRememberedUserSelectedInConversationList() throws Exception {
        final List<String> selectedTopPeople = PeoplePickerPageSteps.getSelectedTopPeople();
        if (selectedTopPeople != null) {
            ...
            oneSelectedTopPeople = usrMgr.replaceAliasesOccurences(oneSelectedTopPeople, FindBy.NAME_ALIAS);
    ...
    public void IClickOnOptionsButton(String contact) throws Exception {
        contact = usrMgr.replaceAliasesOccurences(contact, FindBy.NAME_ALIAS);
        webappPagesCollection.getPage(ContactListPage.class).clickOptionsButtonForContact(contact);
    }
    ...
    public void ISeeCorrectTooltipForSilenceButton() throws Exception {
        ...
        assertThat("Silence button tooltip", webappPagesCollection.getPage(ContactListPage.class).getMuteButtonToolTip(),
            equalTo(tooltip));
    }
    ...
    public void ITypeShortcutCombinationToMuteOrUnmute(String contact) throws Exception {
        contact = usrMgr.replaceAliasesOccurences(contact, FindBy.NAME_ALIAS);
        webappPagesCollection.getPage(ContactListPage.class).pressShortCutToMuteOrUnmute(contact);
    ...
    public void IClickLeaveButton() throws Exception {
        webappPagesCollection.getPage(ContactListPage.class).clickLeaveConversation();
    }
    ...
    public void IClickBlockButton() throws Exception {
        webappPagesCollection.getPage(ContactListPage.class).clickBlockConversation();
    }
    ...
    public void ISeeALeaveWarning() throws Exception {
        Assert.assertTrue(webappPagesCollection.getPage(ContactListPage.class).isLeaveWarningModalVisible());
    }
    ...
    public void IClickCancelButtonOnLeaveWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickCancelOnLeaveWarning();
    }
    ...
    public void ISeeABlockWarning() throws Exception {
        Assert.assertTrue(webappPagesCollection.getPage(ContactListPage.class).isBlockWarningModalVisible());
    }
    ...
    public void IClickCancelButtonOnBlockWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickCancelOnBlockWarning();
    }
    ...
    public void IClickBlockButtonOnBlockWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickBlockOnBlockWarning();
    }
    ...
    public void ISeeConversationWithNameOnTop(String conv) throws Exception {
        ...
        int itemIndex = webappPagesCollection.getPage(ContactListPage.class).getItemIndex(conv);
    ...
    public void IClickDeleteButton() throws Exception {
        webappPagesCollection.getPage(ContactListPage.class).clickDeleteConversation();
    }
    ...
    public void ISeeDeleteWarningForGroup() throws Exception {
        Assert.assertTrue(webappPagesCollection.getPage(ContactListPage.class).isDeleteWarningModalForGroupVisible());
    }
    ...
    public void IClickDeleteButtonOnDeleteWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickDeleteOnDeleteWarning();
    }
    ...
    public void IClickLeaveButtonOnLeaveWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickLeaveOnLeaveWarning();
    }
    ...
    public void IClickLeaveCheckboxOnDeleteWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickLeaveCheckboxOnDeleteWarning();
    }
    ...
    public void IClickCancelButtonOnDeleteWarning() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickCancelOnDeleteWarning();
    }
    ...
    public void ISeeDeleteWarningForSingle() throws Exception {
        Assert.assertTrue(webappPagesCollection.getPage(ContactListPage.class).isDeleteWarningModalSingleVisible());
    }
    ...
    public void IClickDeleteButtonOnDeleteWarningForSingle() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickDeleteOnDeleteWarningSingle();
    }
    ...
    public void IClickCancelButtonOnDeleteWarningForSingle() throws Throwable {
        webappPagesCollection.getPage(ContactListPage.class).clickCancelOnDeleteWarningSingle();
    }
}
