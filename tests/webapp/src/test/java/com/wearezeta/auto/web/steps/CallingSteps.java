...
public class CallingSteps {
     ...
     */
    @When("(.*) calls (.*) using (.*)$")
    public void UserXCallsToUserYUsingCallBackend(String caller, String conversationName, String callBackend) throws Exception {
    ...
    public void UserXStopsCallsToUserY(String caller, String conversationName) throws Exception {
     ...
     *
     * @step. (.*) verif(?:ies|y) that call status to (.*) is changed to (.*) in (\\d+) seconds?$
     *
     * @param caller caller name/alias
    ...
    public void UserXVerifesCallStatusToUserY(String caller, String conversationName, String expectedStatuses,
     ...
     *
     * @step. (.*) verif(?:ies|y) that waiting instance status is changed to (.*) in * (\\d+) seconds?$
     *
    ...
    public void UserXStartsWaitingInstance(String callees, String callingServiceBackend) throws Exception {
    ...
    public void ICallXTimes(int times, int callDurationMinutes, String callees) throws Throwable {
        ...
        final CommonCallingSteps2 commonCalling = CommonCallingSteps2.getInstance();
            ...
            try {
                ...
                try {
                    try {
                        ...
                        warningSteps.IClickButtonInAnotherCallWarningModal("End Call");
        ...
        LOG.info(failures.size() + " failures happened during " + times + " calls");
