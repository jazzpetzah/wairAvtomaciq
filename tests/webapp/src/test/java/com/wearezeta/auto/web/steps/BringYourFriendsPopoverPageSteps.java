...
public class BringYourFriendsPopoverPageSteps {
    ...
    public void ISeeInvitatePeoplePopover(String shouldNotBeVisible) throws Exception {
        if (shouldNotBeVisible == null) {
            webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).isVisible();
        } else {
            webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).isNotVisible();
        }
    }
    ...
    public void ISeeShareContactsButton(String shouldNotBeVisible) throws Exception {
        if (shouldNotBeVisible == null) {
            webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).isShareContactsButtonVisible();
        } else {
            webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).isShareContactsButtonNotVisible();
        }
    }
    ...
    public void IClickShareContactsButton() throws Exception {
        webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).clickShareContactsButton();
    }

    /**
     * Click Invite People button in the Bring Your Friends popover (only visible when having top people and no Gmail imported
     * before)
     * 
    ...
    public void IClickInvitePeopleButton() throws Exception {
        webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).clickInvitePeopleButton();
    }

    /**
     * Save invitation link from the corresponding popover into internal variable
     * 
    ...
    public void IRemeberInvitationLink() throws Exception {
        invitationLink = webappPagesCollection.getPage(BringYourFriendsPopoverPage.class).parseInvitationLink();
    }
    ...
    public void INavigateToNavigationLink() throws Exception {
        if (invitationLink == null) {
            throw new RuntimeException("Invitation link has not been remembered before!");
        }

        YouAreInvitedPage youAreInvitedPage = webappPagesCollection.getPage(YouAreInvitedPage.class);
