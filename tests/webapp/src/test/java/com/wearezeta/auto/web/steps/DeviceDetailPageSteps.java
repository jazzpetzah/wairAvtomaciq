...
public class DeviceDetailPageSteps {
     ...
     * 
     * @param name model of the device
     * @param label label of the device
     * @throws Exception
     */
    @Then("I see a device named (.*) with label (.*) in the device details")
    public void ISeeACertainDeviceInDevicesSection(String name, String label) throws Exception {
        assertThat(webappPagesCollection.getPage(DeviceDetailPage.class).getDeviceLabel(), equalTo(label.toUpperCase()));
        assertThat(webappPagesCollection.getPage(DeviceDetailPage.class).getDeviceLabel(), equalTo(label.toUpperCase()));
    }

    @When("I click the remove device link")
    public void IClickTheRemoveDeviceButton() throws Exception {
        webappPagesCollection.getPage(DeviceDetailPage.class).clickRemoveDeviceLink();
    }

    @When("I type password \"([^\"]*)\" into the device remove form")
    public void ITypePassword(String password) throws Exception {
        webappPagesCollection.getPage(DeviceDetailPage.class).setPassword("aqa123456!");
    }

    @When("I click the remove button")
    public void IClickTheRemoveDeviceButtonOnForm() throws Exception {
        webappPagesCollection.getPage(DeviceDetailPage.class).clickRemoveButton();
    }
}
