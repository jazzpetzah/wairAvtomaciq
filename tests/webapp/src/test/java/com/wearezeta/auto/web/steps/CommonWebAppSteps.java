...
public class CommonWebAppSteps {
    ...
    public static final Logger log = ZetaLogger.getLog(CommonWebAppSteps.class.getSimpleName());
    ...
    public void setUp(Scenario scenario) throws Exception {
        ...
        DesiredCapabilities capabilities = getCustomCapabilities(platform, osName, osVersion, browserName, browserVersion,
            uniqueName);

        final String hubHost = System.getProperty("hubHost");
        final String hubPort = System.getProperty("hubPort");
        final String url = CommonUtils.getWebAppApplicationPathFromConfig(CommonWebAppSteps.class);
        ...
        Callable<ZetaWebAppDriver> callableWebAppDriver = new Callable<ZetaWebAppDriver>() {
            ...
            public ZetaWebAppDriver call() throws Exception {
                ...
                if (WebAppExecutionContext.getBrowser().equals(Browser.Safari)) {
                    WebCommonUtils.closeAllAdditionalTabsInSafari(lazyWebDriver.getNodeIp());
                    WebCommonUtils.clearHistoryInSafari(lazyWebDriver.getNodeIp());
                }
                if (WebAppExecutionContext.getBrowser().isSupportingMaximizingTheWindow()) {
                    lazyWebDriver.manage().window().maximize();
                } else {
                    if (WebAppExecutionContext.getBrowser().isSupportingSettingWindowSize()) {
                        // http://stackoverflow.com/questions/14373371/ie-is-continously-maximizing-and-minimizing-when-test-suite-executes
                        lazyWebDriver
                            .manage()
                            .window()
                            .setSize(
                                new Dimension(WebAppConstants.MIN_WEBAPP_WINDOW_WIDTH, WebAppConstants.MIN_WEBAPP_WINDOW_HEIGHT));
                    }
                }
        ...
        WebappPagesCollection.getInstance().setFirstPage(new RegistrationPage(lazyWebDriver, url));
    ...
    private String getUniqueTestName(Scenario scenario) {
        ...
        String id = scenario.getId().substring(scenario.getId().lastIndexOf(";") + 1);

        return scenario.getName() + " (" + id + ") on " + platform + " with " + browserName + " " + browserVersion;
    }

    private static DesiredCapabilities getCustomCapabilities(String platform, String osName, String osVersion,
        ...
        switch (browser) {
            ...
            case MicrosoftEdge:
                ...
                break;
            default:
                throw new NotImplementedException("Unsupported/incorrect browser name is set: " + browserName);
        }

        if (browser.isSupportingConsoleLogManagement()) {
            setExtendedLoggingLevel(capabilities, WebCommonUtils.getExtendedLoggingLevelInConfig(CommonCallingSteps2.class));
        }
    ...
    public void ThereAreNUsersWhereXIsMeWithoutEmail(int count, String myNameAlias) throws Exception {
        commonSteps.ThereAreNUsersWhereXIsMeWithPhoneNumberOnly(CURRENT_PLATFORM, count, myNameAlias);
    }
    ...
    public void IChangeUserAvatarPicture(String userNameAlias, String path) throws Exception {
        ...
        log.debug("Change avatar of user " + userNameAlias + " to " + uri.getPath());
    ...
    public void UserSendMessageToConversation(String msgFromUserNameAlias, String isEncrypted, String msg, String convoType,
    ...
    public void UserSentMessageToConversation(String userFromNameAlias, String message, String conversationName)
        throws Exception {
    ...
    public void IVerifyUserReceiverInvitation(String alias) throws Throwable {
        ...
        assertTrue(String.format("Invitation email for %s is not valid", email), BackendAPIWrappers.getInvitationMessage(email)
            .orElseThrow(() -> {
    ...
    public void INavigateToPersonalInvitationRegistrationPage(String alias) throws Throwable {
        final String email = usrMgr.findUserByNameOrNameAlias(alias).getEmail();
        String url = BackendAPIWrappers.getInvitationMessage(email).orElseThrow(() -> {
            throw new IllegalStateException("Invitation message has not been received");
        }).extractInvitationLink();
        RegistrationPage registrationPage = WebappPagesCollection.getInstance().getPage(RegistrationPage.class);
        ...
        if (PlatformDrivers.getInstance().hasDriver(CURRENT_PLATFORM)) {
            try {
                if (WebAppExecutionContext.getBrowser().isSupportingConsoleLogManagement()) {
                    List<LogEntry> browserLog = getBrowserLog(PlatformDrivers.getInstance().getDriver(CURRENT_PLATFORM)
                        .get(DRIVER_INIT_TIMEOUT, TimeUnit.MILLISECONDS));

                    StringBuilder bLog = new StringBuilder("\n");
                    browserLog.stream().forEach((entry) -> {
                        bLog.append(entry.getLevel()).append(":").append(entry.getMessage()).append("\n");
                    });
                    assertTrue("BrowserLog is not empty: " + bLog.toString(), browserLog.isEmpty());
                }
    ...
    public void IRefreshPage() throws Exception {
        WebappPagesCollection.getInstance().getPage(RegistrationPage.class).refreshPage();
    }
    ...
    public void tearDown(Scenario scenario) throws Exception {
        ...
        if (webdrivers.containsKey(uniqueName)) {
            ...
            try {
                ...
                String link = "https://saucelabs.com/jobs/" + driver.getSessionId();
                ...
                scenario.embed(html.getBytes(Charset.forName("UTF-8")), "text/html");
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
    ...
    public void ContactSendImageToConversation(String imageSenderUserNameAlias, String imageFileName, String conversationType,
    ...
    public void UserUnblocks(String userAsNameAlias, String userToBlockNameAlias) throws Exception {
    ...
    public void IOpenSignInPage() throws Exception {
        WebappPagesCollection.getInstance().getPage(RegistrationPage.class).openSignInPage();
    }
