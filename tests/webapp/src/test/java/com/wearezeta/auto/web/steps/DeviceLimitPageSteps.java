...
public class DeviceLimitPageSteps {
    ...
    public void IClickSignOutButton() throws Exception {
        webappPagesCollection.getPage(DeviceLimitPage.class).clickSignOutButton();
    }

    @When("I click button to manage devices$")
    public void IClickManageDevicesButton() throws Exception {
        webappPagesCollection.getPage(DeviceLimitPage.class).clickManageDevicesButton();
    }

    @Then("I am informed about the device limit$")
    public void IAmInformedAboutDeviceLimit() throws Exception {
        assertThat("Device limit info not shown", webappPagesCollection.getPage(DeviceLimitPage.class).isDeviceLimitInfoShown());
    }
    ...
    public void ISeeACertainDevice(String name, String label) throws Exception {
        assertThat(webappPagesCollection.getPage(DeviceLimitPage.class).getDevicesNames(), hasItem(name.toUpperCase()));
    }

    @Then("I see (\\d+) devices under managed devices$")
    public void ISeeXDevices(int size) throws Exception {
        assertThat(webappPagesCollection.getPage(DeviceLimitPage.class).getDevicesNames(), hasSize(size));
    }
}
