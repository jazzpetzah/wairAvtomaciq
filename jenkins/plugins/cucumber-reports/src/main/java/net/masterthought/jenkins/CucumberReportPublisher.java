...
public class CucumberReportPublisher extends Recorder {
	...
	public CucumberReportPublisher(String jsonReportDirectory, String pluginUrlPath, boolean skippedFails,
			boolean undefinedFails, boolean noFlashCharts, boolean ignoreFailedTests, String realBuildNumber,
			String coverage, String customDescription) {
	...
	public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
		...
		if (Computer.currentComputer() instanceof SlaveComputer) {
			listener.getLogger()
					.println("[CucumberReportPublisher] copying all json files from slave: "
							+ workspaceJsonReportDirectory.getRemote() + " to master reports directory: "
							+ targetBuildDirectory);
		} else {
			listener.getLogger().println("[CucumberReportPublisher] copying all json files from: "
		...
		if (jsonReportFiles.length != 0) {
			...
			listener.getLogger().println(
					"[CucumberReportPublisher] Found the following number of json files: " + jsonReportFiles.length);
			int jsonIndex = 0;
			for (String jsonReportFile : jsonReportFiles) {
				listener.getLogger()
						.println("[CucumberReportPublisher] " + jsonIndex + ". Found a json file: " + jsonReportFile);
				jsonIndex++;
			}
			...
			try {
				ReportBuilder reportBuilder = new ReportBuilder(
						fullPathToJsonFiles(jsonReportFiles, targetBuildDirectory), targetBuildDirectory,
						parsedCustomDescription, parsedCoverage, parsedRealBuildNumber, pluginUrlPath, buildNumber,
						buildProject, skippedFails, undefinedFails, !noFlashCharts, true, false, "", false);
			...
			result = Result.SUCCESS;
			listener.getLogger()
					.println("[CucumberReportPublisher] there were no json results found in: " + targetBuildDirectory);
		}
